#!/usr/bin/env bash
###############################################################################
# run_backbone_v11.sh - v11 scaled re-run driver.
#
# Same six-stage pipeline as run_backbone.sh, but:
#   * namespace  : constraint_guided_v11_${RUN_TAG}  (v10 preserved untouched)
#   * datasets   : SpartQA-YN DROPPED (chance-level for all methods). Headline is
#                  StepGame(ID) + spartqa, babi, SpaRTUN, SpaceNLI (OOD).
#   * scale      : train 5000 / val 2000 / test 3000 (kills ties/base-rate collapse)
#   * combiner   : scripts/fusion.py (k-fold CV routing + Brier-aware calibration)
#
# Backbone + tag come from the per-model wrapper (run_{llama,mistral,gemma,phi4}_v11.sh).
# Phi uses phi-4 (non-reasoning); Phi-4-reasoning had degenerate single-class labels.
#
...
# Head zoo trained on the ID cache (same set as v10 headline).
...
echo "########################################################"
echo "### run_backbone_v11 start $(date)"
echo "### backbone : ${MODEL_NAME}"
echo "### tag      : ${RUN_TAG}   namespace: constraint_guided_v11_${RUN_TAG}"
echo "### cache    : ${ID_CACHE}"
...
echo "########################################################"
echo "### run_backbone_v11 (${RUN_TAG}) done $(date)"
echo "########################################################"
