#!/usr/bin/env bash
###############################################################################
# run_v10_backbone.sh - v10 pipeline for an ARBITRARY backbone.
#
# Identical protocol / sample sizes to run_v10.sh (the Llama-3.1-8B run), but the
# backbone, cache namespace, results root and logs root are parameterized so
# additional backbones can be run WITHOUT overwriting the original v10 artifacts.
#
# Usage (do not run directly; use the per-model wrappers run_v10_<model>.sh):
#   MODEL_NAME=Mistral-7B-Instruct-v0.3 \
#   RUN_TAG=mistral7b \
#   bash jobs/run_v10_backbone.sh
#
# Required env in:  MODEL_NAME, RUN_TAG
# Everything else defaults to the exact v10 settings.
###############################################################################
set -uo pipefail
...
: "${RUN_TAG:?set RUN_TAG (short slug for the namespace, e.g. mistral7b)}"

# --- namespace: keyed by RUN_TAG so nothing collides with the Llama v10 run ---
...
# --- same scaled sizes as the Llama v10 run ---
...
echo "########################################################"
echo "### run_v10_backbone start $(date)"
...
echo "########################################################"
echo "### run_v10_backbone (${RUN_TAG}) done $(date)"
echo "########################################################"
